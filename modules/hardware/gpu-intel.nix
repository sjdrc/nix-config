{...}: {
  flake.nixosModules.gpu-intel = {config, lib, pkgs, ...}: {
    options.custom.gpu-intel.enable = lib.mkEnableOption "Intel GPU support";

    config = lib.mkIf config.custom.gpu-intel.enable (let
