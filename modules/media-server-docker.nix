{...}: {
  flake.nixosModules.media-server-docker = {config, lib, ...}: let
    cfg = config.custom.media-server-docker;
  ...
  in {
    options.custom.media-server-docker.enable = lib.mkEnableOption "Docker-based media server suite";

    config = lib.mkIf cfg.enable {
      assertions = [
        {
          assertion = !(config.custom.media-server.enable or false);
